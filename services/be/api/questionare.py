...
def add_question(
    ...
    try:
        record = crud.insert(
            db, table_name="Question", data={"question_text": question_text}
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"question already exits!"
        )
...
def add_question_option(
    ...
    record = crud.get_record_by_param(
        db, table_name="Question", param_name="id", param_value=option.question_id
    )
    if not record:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"question_id is invalid!"
        )
    try:
        ...
        log.error(f"Create: IntegrityError: {e.orig}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"option already exits!"
        )
...
def delete_question(
    ...
    record = crud.delete_record(
        db, table_name="Question", param_name="id", param_value=id
    )
    if record:
        ...
        raise HTTPException(
            ...
            detail=f"Invalid question id supplied!",
        )
